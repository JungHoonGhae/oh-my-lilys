...
export async function showLanguage(options: { json?: boolean } = {}) {
  const stored = getResultLanguage();
  let serverLang: string | null = null;

  if (await isAuthenticated()) {
    try {
      const prefs = await getUserPreferences();
      serverLang = prefs.userPreference?.resultLanguage || null;
    } catch {
      // ignore
    }
  }
  ...
  if (options.json) {
    console.log(JSON.stringify({ local: stored, server: serverLang }, null, 2));
    return;
  }

  logger.log(`${logger.bold("Current AI Result Language:")} ${styles.value(stored)}`);
  logger.log(`${logger.bold("Server setting:")} ${serverLang || styles.dim("(not set)")}`);
}

export async function setResultLanguageCmd(lang: string, options: { json?: boolean } = {}) {
  if (!(await isAuthenticated())) {
  ...
  try {
    ...
    if (options.json) {
      console.log(JSON.stringify({ language: lang, updated: true }, null, 2));
      return;
    }
    ...
    if (options.json) {
      console.log(JSON.stringify({ language: lang, updated: true, serverSync: false }, null, 2));
      return;
    }
